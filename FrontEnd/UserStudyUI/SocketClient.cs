...
namespace DefaultNamespace
{
	...
	public class SocketClient : System.Windows.Forms.Form
	{
		...
		byte[] m_dataBuffer = new byte [10*1024];
		...
		private void InitializeComponent() {
			...
			this.richTextTxMessage.WordWrap = false;
			// 
			...
			this.richTextRxMessage.WordWrap = false;
			// 
			// textBoxPort
			// 
		...
		void ButtonConnectClick(object sender, System.EventArgs e)
		{
			...
			{
				// Output silenced for auto-connect
		...
		void ButtonSendMessageClick(object sender, System.EventArgs e)
		{
			try
			{
				...
				this.richTextTxMessage.AppendText("Sending " + this.txStr.Length + " chars.\n");
		...
		public class SocketPacket
		{
			...
			public byte[] dataBuffer = new byte[10*1024];
		}
		public void OnDataReceived(IAsyncResult asyn)
		{
			try
			{
				...
				char[] chars = new char[iRx]; // OBOB!! is iRx + 1 in online version
				System.Text.Decoder d = System.Text.Encoding.UTF8.GetDecoder();
				int charLen = d.GetChars(theSockId.dataBuffer, 0, iRx, chars, 0);
				System.String szData = new System.String(chars);
				this.rxStr += szData;
				richTextRxMessage.AppendText("Received data: " + szData.Length + " chars");
				WaitForData();
				ResultReceived(); // Sends XML result to User Study UI when necessary
			}
	   ...
	   {	   
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// Additional Features for User Study Form Integration
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		#region Additional Features for User Study Form Integration


		/// <summary>
		/// IP addresses of development machine (DevIP) and wizard machine (WizardIP)
		/// </summary>
		public static string WizardIP = "127.0.0.1";
		//public static string WizardIP = "134.173.200.159";
		//public static string WizardIP = "192.168.0.1";
		//public static string WizardIP = "134.173.60.63";


		/// <summary>
		/// Delegate for sending XML in a separate thread.
		/// </summary>
		public delegate void SocketClientSendXml(string xml, bool userTriggered);
		
		/// <summary>
		/// Event for sending XML results to the Wizard Recognizer
		/// <see cref="UserStudyUI.WizardRecognizer"/>
		/// </summary>
		public event UserStudyUI.WizardResultReceivedEventHandler SketchRecievedEvent;

		/// <summary>
		/// The XML end tag to look for in a recieved text message
		/// </summary>
		public static string SketchEndTag = "</sketch>";

		/// <summary>
		/// The tag to send ahead of a sketch when the user triggers
		/// recognition for a sketch.
		/// </summary>
		public static string UserTriggeredTag = "<USERTRIGGERED>";

		/// <summary>
		/// String buffers for holding transmitted and received data;
		/// faster than using the original RichTextBoxes
		/// </summary>
		private string rxStr = "";
		private string txStr = "";

		/// <summary>
		/// Send text (e.g., sketch XML) through the
		/// SocketClient.
		/// </summary>
		/// <param name="xml">the xml string to send</param>
		/// <param name="userTriggered">true iff the user triggered recognition</param>
		public void SendXml(string xml, bool userTriggered)
		{
			if (userTriggered)
			{
				this.txStr += UserTriggeredTag;
			}

			this.txStr += xml;
			this.ButtonSendMessageClick(null, null);
			this.txStr = "";
		}

		/// <summary>
		/// Searches the recieved text for a SketchEndTag and 
		/// sends the recieved XML to the Wizard Recognizer when
		/// transmission is complete.
		/// </summary>
		private void ResultReceived()
		{
			string textRecieved = this.rxStr;
			int searchLength = SketchEndTag.Length + 4;  // pad end tag search window by 4 characters

			if (textRecieved.Length < (searchLength + 1))
			{
				// Result is too short to contain an
				// entire sketch
				return;
			}

			// Check for </sketch> end tag in text
			if (textRecieved.IndexOf(SketchEndTag, textRecieved.Length - searchLength) > 0)
			{	
				SketchRecievedEvent(textRecieved);

				// Now reset the text recieved field
				this.rxStr = "";
			}
		}

		/// <summary>
		/// Starts the SocketClient (used by UI for client auto-start)
		/// </summary>
		public void startSocketClient()
		{
			textBoxIP.Text = WizardIP;
			this.ButtonConnectClick(null, null);
		}

		/// <summary>
		/// User hook for restarting the client if the connection fails
		/// </summary>
		public void restartSocketClient()
		{
