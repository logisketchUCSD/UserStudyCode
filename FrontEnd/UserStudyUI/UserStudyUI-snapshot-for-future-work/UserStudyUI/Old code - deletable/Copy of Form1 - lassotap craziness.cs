...
namespace UserStudyUI
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		/// <summary>
		/// Tracks the state of gesture recognition.  When the 
		/// lasso-tap trigger is enabled, the rest of the form
		/// can be notified of this state using the current mode
		/// variable.
		/// </summary>
		private enum GestureState {Disabled, LassoTapSelect_Lasso, LassoTapSelect_Tap}
		private GestureState currentGestureState = GestureState.Disabled;
		...
		public Form1()
		{
			...
			// TODO Configure Ink Picture properties (turn on time collection, etc)
			sketchPicture.EraserMode = InkOverlayEraserMode.StrokeErase;
			notesPicture.EraserMode = InkOverlayEraserMode.StrokeErase;
		...
		private void lassoRadio_CheckedChanged(object sender, System.EventArgs e)
		{
			...
			{
				currentGestureState = GestureState.LassoTapSelect_Lasso;
				sketchPicture.Selection.Clear();
				sketchPicture.EditingMode = InkOverlayEditingMode.Select;
			} 
			else // unchecked
			{
				if (currentGestureState == GestureState.LassoTapSelect_Tap)
				{
		...
		/// <summary>
		/// Zooms in on the sketch (making the sketch appear larger
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		...
		/// <summary>
		/// Zoom out on the sketch (making the sketch appear smaller
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		...
		{
			// If have gesture recognition enabled, then we don't necessarily want to record the selection stroke
			if (currentGestureState ==  GestureState.Disabled)
			{
				Sketch.Stroke stroke = readJnt.InkStroke2SketchStroke(e.Stroke, null, false);
				sketch.AddStroke(stroke);
			} 
			else if (currentGestureState == GestureState.LassoTapSelect_Tap)
			{
				// If the tap gesture is enabled but we see a stroke, then 
				// this probably means that the user unsuccessfully completed
				// the tap gesture.  Thus we cancel the tap gesture collection.
				this.disableTapGesture();
			}
		}
		...
		/// <summary>
		/// Handles stroke additions passed to the sketch panel
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void inkAdded2Sketch(object sender, StrokesEventArgs e) 
		{
			...
			// Fire the pause trigger, if applicable
			if (currentTrigger == Trigger.Pause) 
			{
		...
		/// <summary>
		/// Handles stroke deletions passed to the sketch panel
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		...
		/// <summary>
		/// Handles ink additions passed to the notes panel
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		...
		/// <summary>
		/// Handles ink deletions passed to the notes panel
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		...
		{
			// Save the selected strokes
			this.lastSelection = sketchPicture.Selection;
			
			// TODO Send a false tap outside of the selection to clear it
			//int tapX = sketchPicture.Selection.GetBoundingBox().X - 10;
			//int tapY = sketchPicture.Selection.GetBoundingBox().Y - 10;

			// Move to tap gesture state
			sketchPicture.EditingMode = InkOverlayEditingMode.Ink;
			this.enableTapGesture();
			currentGestureState = GestureState.LassoTapSelect_Tap;
		}
		...
		{
			sketchPicture.CollectionMode = CollectionMode.GestureOnly;
		...
		private void sketchPicture_LassoTapWithSelectGesture(object sender, InkCollectorGestureEventArgs e)
		{
			...
			{
				currentGestureState = GestureState.Disabled;
			...
			{
				// Cancel tap gesture, but erase the ink
				e.Cancel = false;
