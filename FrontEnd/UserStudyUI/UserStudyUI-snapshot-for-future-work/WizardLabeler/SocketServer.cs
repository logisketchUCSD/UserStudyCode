...
namespace DefaultNamespace
{
	...
	public class SocketServer : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent() {
			...
			this.richTextBoxSendMsg.WordWrap = false;
			// 
			// label1
			// 
			...
			this.richTextBoxReceivedMsg.WordWrap = false;
			// 
			// label2
			// 
		...
		public void OnDataReceived(IAsyncResult asyn)
		{
			try
			{
				...
				// Send data to internal buffer string instead of 
				// RichTextBox
				rxStr += szData;
				richTextBoxReceivedMsg.AppendText("Received message of size " + szData.Length + ".\n");
	
				// Continue the waiting for data on the Socket
				WaitForData( socketData.m_currentSocket );

				// Send XML data to the Labeler when necesary
				SketchXMLReceived();
			}
			catch (ObjectDisposedException )
			{
	   ...
	   void CloseSockets()
	   {
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// Additional Features for Wizard Labeler Form Integration
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		#region Additional Features for Wizard Labeler Form Integration

		/// <summary>
		/// Event for sending received sketch XML to the Labeler.
		/// </summary>
		public event Labeler.Form1.NetworkSketchReceivedEventHandler NetworkSketchReceivedEvent;

		/// <summary>
		/// The XML end tag to look for in a received text message
		/// </summary>
		...
		/// <summary>
		/// Sends text (e.g., sketch XML) to all connected
		/// SocketClients.
		/// </summary>
		public string SendText
		{
			...
			{
				this.txStr = value;
				this.ButtonSendMsgClick(null, null);
				this.txStr = "";
			}
		}

		/// <summary>
		/// Searches the received text for a SketchEndTag and 
		/// sends the received XML to the Labeler when
		/// transmission is complete.
		/// </summary>
		private void SketchXMLReceived()
		{
			string textReceived = this.rxStr;
			int searchLength = SketchEndTag.Length + 4;  // pad end tag search window by 4 characters

			if (textReceived.Length < (searchLength + 1))
			{
			...
			if (textReceived.IndexOf(SketchEndTag, textReceived.Length - searchLength) > 0)
			{	
				bool userTriggered = false;

				// Check for UserTriggered tag
				if (textReceived.IndexOf(UserTriggeredTag, 0, UserTriggeredTag.Length + 4) != -1)
				{
					userTriggered = true;
					textReceived = textReceived.Remove(0, UserTriggeredTag.Length);
				}

				// Send event to labeler
				NetworkSketchReceivedEvent(textReceived, userTriggered);

				// Now reset the text received field
				this.rxStr = "";
			}
		}

		/// <summary>
		/// Starts the SocketServer (provides public access to start function).
		/// </summary>
		public void startSocketServerListen()
		{
			this.ButtonStartListenClick(null, null);
		}
