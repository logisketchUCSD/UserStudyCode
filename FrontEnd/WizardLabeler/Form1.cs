...
namespace Labeler
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		/// </summary>
		private Dictionary<int, Substroke> mIdToSubstroke;

		/// <summary>
		/// Hashtable from Microsoft Stroke Ids to their corresponding indices in the InkOverlay.
		/// </summary>
		private Dictionary<int, int> mIdToInkIndex;
		...
		/// <summary>
		/// The SocketServer that sends XML of labeled sketches to User Study UI clients
		/// </summary>
		private DefaultNamespace.SocketServer socketServer;

		/// <summary>
		/// The most recent sketch received from the user
		/// </summary>
		private string cachedSketchXml;

		/// <summary>
		/// Stores GUIDs of all the Sketch.Strokes that the Labeler has split.
		/// </summary>
		private List<Guid?> splitStrokes;

		/// <summary>
		/// Default location of standard logic gates domain file.
		/// </summary>
		public static string DefaultDomainFilePath = ".\\LogicGatesColorDomain.txt";

		/// <summary>
		/// Statistics used to notify Wizard of when to trigger errors
		/// </summary>
		//private int numLabelsCompleted = 0;
		//private double targetErrorRate = 0.15;
		//private int expectedShapeCount = 30;
		...
		private Genghis.Windows.Forms.CompletionCombo shapeLabels;
		...
		private System.Windows.Forms.MenuItem sendXmlMI;
		private System.Windows.Forms.MenuItem refreshXmlMI;
		private System.Windows.Forms.MenuItem applyLabelMI;
		private System.Windows.Forms.MenuItem toggleSocketServerMI;
		...
		public Form1()
		{
			...
			this.mIdToIndices	  = new Dictionary<int, List<float>>();
			this.mIdToSubstroke	  = new Dictionary<int, Substroke>();
			this.mIdToInkIndex	  = new Dictionary<int, int>();
			this.splitStrokes	  = new List<Guid?>();
			...
			// Initial GUI mode is Selection
			changeModeSelect();

			// Create Socket Server with GUI for sending XML results
			this.socketServer = new DefaultNamespace.SocketServer();
			this.socketServer.NetworkSketchReceivedEvent +=	new NetworkSketchReceivedEventHandler(networkSketchReceived);
			this.socketServer.startSocketServerListen();
			//this.socketServer.Show();

			// Create empty sketch; this will be overwritten once the Labeler opens a file
			// or a sketch is received from the network
			this.sketch = new Sketch.Sketch();

			// Attempt to load domain
			if (File.Exists(DefaultDomainFilePath))
			{
				StreamReader sr = new StreamReader(DefaultDomainFilePath);
				loadDomain(sr);
				sr.Close();
		...
		private void InitializeComponent()
		{
			...
			this.sendXmlMI = new System.Windows.Forms.MenuItem();
			this.refreshXmlMI = new System.Windows.Forms.MenuItem();
			this.toggleSocketServerMI = new System.Windows.Forms.MenuItem();
			this.menuItem1 = new System.Windows.Forms.MenuItem();
			this.exitMenuItem = new System.Windows.Forms.MenuItem();
			this.menuItem3 = new System.Windows.Forms.MenuItem();
			this.undoMI = new System.Windows.Forms.MenuItem();
			this.redoMI = new System.Windows.Forms.MenuItem();
			this.applyLabelMI = new System.Windows.Forms.MenuItem();
			...
			this.shapeLabels = new Genghis.Windows.Forms.CompletionCombo();
			...
			this.saveXmlMI.Shortcut = System.Windows.Forms.Shortcut.CtrlShiftS;
			...
			// sendXmlMI
			// 
			this.sendXmlMI.Index = 5;
			this.sendXmlMI.Shortcut = System.Windows.Forms.Shortcut.CtrlS;
			this.sendXmlMI.Text = "Send XML";
			this.sendXmlMI.Click += new System.EventHandler(this.sendXmlMi_Click);
			// 
			// refreshXmlMI
			// 
			this.refreshXmlMI.Index = 6;
			this.refreshXmlMI.Shortcut = System.Windows.Forms.Shortcut.CtrlR;
			this.refreshXmlMI.Text = "Refresh XML";
			this.refreshXmlMI.Click += new System.EventHandler(this.refreshXmlMI_Click);
			// 
			// toggleSocketServerMI
			// 
			this.toggleSocketServerMI.Enabled = false;
			this.toggleSocketServerMI.Index = 7;
			this.toggleSocketServerMI.Text = "Toggle SocketServer";
			this.toggleSocketServerMI.Click += new System.EventHandler(this.toggleSocketServerMI_Click);
			// 
			// menuItem1
			// 
			...
			// applyLabelMI
			// 
			this.applyLabelMI.Index = 2;
			this.applyLabelMI.Shortcut = System.Windows.Forms.Shortcut.CtrlA;
			this.applyLabelMI.Text = "Apply Label";
			this.applyLabelMI.Click += new System.EventHandler(this.applyLabelMI_Click);
			// 
			// statusBar
			// 
			...
			// shapeLabels
			// 
			this.shapeLabels.LimitToList = true;
			...
			this.commitBtn.Location = new System.Drawing.Point(288, 500);
			...
			this.openXmlBtn.Text = "Refresh XML";
		...
		private void labelButton_Click(object sender, System.EventArgs e)
		{
			...
			if (oInk.EditingMode == InkOverlayEditingMode.Select && oInk.Selection.Count > 0)
			{
				...
				{
					selected.Add(this.mIdToSubstroke[stroke.Id]);
				}
		...
		/// <summary>
		/// Hotkey for applying labels: Ctrl+A
		/// </summary>
		private void applyLabelMI_Click(object sender, System.EventArgs e)
		{
			this.labelButton_Click(null, null);
		}
		...
		private void drawEndPts(Graphics g, Color color)
		{
			...
			highlightPoints(g, pts.ToArray(), color);
		}
		...
		{
			foreach(KeyValuePair<int, List<float>> entry in mIdToIndices)
			{
				int mId = entry.Key;

				float[] indices = entry.Value.ToArray();

				Microsoft.Ink.Stroke s = oInk.Ink.Strokes[ mIdToInkIndex[mId] ];
		...
		private void updateColorsFromLabels()
		{
			if (this.domainInfo != null && this.sketch != null)
			{
				...
				{
					string[] labels = substrokes[i].Labels;
...
//				}
		...
		private void splitAll()
		{
			...
			foreach (KeyValuePair<int, List<float>> mId in this.mIdToIndices)
			{
				// Get the float array of indices where we should split a stroke
				float[] fIndices = mId.Value.ToArray();
				...
				// Note the index of the split stroke for networking functionality
				// see refreshXmlMI_Click()
				splitStrokes.Add(currSubstroke.XmlAttrs.Id);
		...
		/// <summary>
		/// Callback that handles sketches from the User.  The SocketServer
		/// forwards each full sketch XML received to the Labeler.  This method 
		/// unmarshalls the received sketch and caches the new sketch internally.
		/// </summary>
		/// <param name="sketchXml">The XML string that the User sent.</param>
		/// <param name="userTriggered">True iff the user triggered recognition</param>
		private void networkSketchReceived(string sketchXml, bool userTriggered)
		{
			// Cache the new Sketch data
			this.cachedSketchXml = sketchXml;

			// Update UI with new status text
			string newStatus;
			if (userTriggered)
				newStatus = "USER TRIGGERED RECOGNITION";
			else
				newStatus = "New Strokes Available";
			
			StatusUpdateHandler d = new StatusUpdateHandler(this.updateStatusText);
			this.Invoke(d, new object[] { newStatus });

		}
		
		/// <summary>
		/// Sends the current sketch to the User via the SocketServer.
		/// </summary>
		private void sendXmlMi_Click(object sender, System.EventArgs e)
		{
			this.statusBar.Text = "Sending sketch...";

			// Create XML from sketch and send the XML
			MakeXML makeXml = new MakeXML(this.sketch);
			this.socketServer.SendText = makeXml.getXMLstr();

			this.statusBar.Text = "Sketch sent.";
		}

		/// <summary>
		/// Refreshes the Labeler display with the latest sketch received from the user.
		/// Merges labeled and segmented strokes between the current and new sketches.
		/// </summary>
		private void refreshXmlMI_Click(object sender, System.EventArgs e)
		{
			//this.statusBar.Text = "Refreshing sketch...";

			//
			// Read the XML string and cache the new sketch
			//
			Sketch.Sketch cachedSketch = new Sketch.Sketch();
			XmlTextReader xmlReader = new XmlTextReader(new StringReader(this.cachedSketchXml));
			xmlReader.WhitespaceHandling = WhitespaceHandling.None;
			try 
			{
				ReadXML readXml = new ReadXML(xmlReader);
				cachedSketch = readXml.Sketch;
			} 
			catch (Exception ex) 
			{
				...
				errorWriter.Write(this.cachedSketchXml);
				//Console.WriteLine(this.cachedSketchXml);
			}
			

			//
			// Set Sketch Paramters
			//

			this.sketch.XmlAttrs.Id = cachedSketch.XmlAttrs.Id;
			this.sketch.XmlAttrs.Units = cachedSketch.XmlAttrs.Units;
			...
			// Handle split strokes.
			// Split strokes always have more than one substroke. 
			// Thus first collect the GUIDs of all substrokes that
			// belong to a stroke that has more than one 
			// substroke.  These substrokes have been
			// spawned through splitting.
			List<Guid?> spawnedStrokes = new List<Guid?>();
			Sketch.Substroke[] sketchSStrokes = this.sketch.Substrokes;
			for (int i = 0; i < sketchSStrokes.Length; ++i)
			{
				if (sketchSStrokes[i].ParentStroke.Substrokes.Length > 1)
					spawnedStrokes.Add(sketchSStrokes[i].XmlAttrs.Id);
			}
			
			// Delete all substrokes from the current sketch that are not
			// in the cached sketch and that are not spawned from splitting
			Substroke cachedSStroke;
			for (int i = 0; i < sketchSStrokes.Length; ++i)
			{
				// If this is a spawned stroke, skip it
				if (spawnedStrokes.Contains(sketchSStrokes[i].XmlAttrs.Id))
					continue;

				// Search for matching substroke
				cachedSStroke = cachedSketch.GetSubstroke((System.Guid)sketchSStrokes[i].XmlAttrs.Id);

				// If there is no matching substroke, delete this one from the current sketch
				// In this case, it is likely that the user erased this substroke.
				if (cachedSStroke == null)
				{
					this.sketch.RemoveSubstroke(sketchSStrokes[i]);
				}
			}

			// Now add any new strokes from the cached sketch to the current sketch.
			foreach (Sketch.Stroke curCacheStroke in cachedSketch.Strokes)
			{
				if (this.sketch.GetStroke((System.Guid)curCacheStroke.XmlAttrs.Id) == null)
				{
					this.sketch.AddStroke(curCacheStroke);
				}
			}
			...
			//this.statusBar.Text = "Sketch refreshed.";
		}

		/// <summary>
		/// Delegate for receiving new sketches to label from a networked user.
		/// </summary>
		public delegate void NetworkSketchReceivedEventHandler(string sketchXml, bool userTriggered);

		/// <summary>
		/// Delegate for updating the status bar from the SocketServer thread.  When a 
		/// sketch is received, the SocketServer creates a new thread which cannot
		/// modify the UI, thus the SocketServer must send a status update using
		/// this delegate.
		/// See in UserStudyUIForm.cs region "Hooks for Wizard Recognizer + Text Labels"
		/// </summary>
		public delegate void StatusUpdateHandler(string statusText);

		/// <summary>
		/// Updates the status text.
		/// <see cref="StatusUpdateHandler"/>
		/// </summary>
		...
		/// <summary>
		/// Toggles the visibility of the SocketServer
		/// </summary>
		private void toggleSocketServerMI_Click(object sender, System.EventArgs e)
		{
			this.socketServer.Visible = !toggleSocketServerMI.Checked;
			toggleSocketServerMI.Checked = !toggleSocketServerMI.Checked;
		}
