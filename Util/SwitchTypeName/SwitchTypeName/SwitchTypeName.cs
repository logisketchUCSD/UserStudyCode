    ...
    class SwitchTypeName
    {
        ...
        static string[] getFiles(ArrayList argArray)
        {
            ...
            if (argArray.Contains("-i"))
                SwitchTypeName.endExtension = "*." + (string)argArray[argArray.IndexOf("-i") + 1];
          
            if (argArray.Contains("-o"))
                SwitchTypeName.newExtension = "." + (string)argArray[argArray.IndexOf("-o") + 1];

            if (argArray.Contains("-c")) // Convert everything in this directory
            {
                files = Directory.GetFiles(Directory.GetCurrentDirectory(), SwitchTypeName.endExtension);
            }
            else if (argArray.Contains("-d")) // Convert everything in specified directory
            {
                ...
                else if (!Directory.Exists((string)argArray[argArray.IndexOf("-d") + 1])) // Does dir exist?
                {
                    ...
                    files = Directory.GetFiles((string)argArray[argArray.IndexOf("-d") + 1], SwitchTypeName.endExtension);
            }
            else if (argArray.Contains("-r")) //Recursive from current dir
            {
                ...
                string[] currDir = Directory.GetFiles(Directory.GetCurrentDirectory(), SwitchTypeName.endExtension);
        ...
        {
            string outputFilename = filename.Remove(filename.Length - SwitchTypeName.endExtension.Length + 1) + SwitchTypeName.newExtension;
        ...
        static void DirSearch(string sDir, ref ArrayList rFiles)
        {
            try
            {
                ...
                {
                    foreach (string f in Directory.GetFiles(d, SwitchTypeName.endExtension))
                    {
                        rFiles.Add(f);
                    }
